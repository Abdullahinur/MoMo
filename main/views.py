...
def create_welcome_message(new_user):
    master, master_status = User.objects.get_or_create(
        user_name="Larry Georges", user_email="host@test.com", user_password="12345")
    init_contact, contact_status = Contact.objects.get_or_create(
        user_reference=new_user, contact_id=master.id, contact_name=master.user_name, contact_email=master.user_email)
    init_chat, chat_status = Conversation.objects.get_or_create(contact_reference=init_contact, contact_id=init_contact.id, contact_email=init_contact.contact_email,
                                                                contact_name=init_contact.contact_name, msg_content='Welcome to the FaceBook Clone <br />♦ Have a pleasant experience ☻ ♦')
    ...
    if verified_user.contact_set.all():
        for contact in verified_user.contact_set.all():
            if contact.conversation_set.all():
                unread_status = contact.conversation_set.filter(
                    has_been_read=False)
                if unread_status:
...
def signUp(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # saving to database
            new_user = User(user_name=captured_user_name,
                            user_email=captured_user_email, user_password=captured_user_password)
            new_user.save()
...
def profile(request):
    ...
    has_unread_messages, unread_contact_id = check_for_unread_messages(
        verified_user)
...
def messager(request):
    ...
    if change_status == 'read':
        try:
            ...
            filtered_tread = contact_from_msg.conversation_set.filter(
                has_been_read=False)
            if filtered_tread:
    ...
    if change_status == 'response':
        try:
            contact_from_msg = Contact.objects.get(id=str(change_item))
            Conversation.objects.create(contact_reference=contact_from_msg, contact_id=contact_from_msg.id, contact_email=contact_from_msg.contact_email,
                                        contact_name=contact_from_msg.contact_name, msg_content=message_item, has_been_read=True)
        except Exception as error:
            print(error)
...
def edit_info(request):
    ...
    if request.method == 'POST':
        ...
        # saving to database
        verified_user.user_options_set.create(
            about_bio=captured_user_about, about_location=captured_user_location)
...
def make_post(request):
    ...
    if request.method == 'POST':
        ...
        # saving to database
        verified_user.user_posts_set.create(
            post_made=captured_user_post, pub_date=datetime.datetime.now(), likes=0)
...
def like_post(request):
    ...
    if request.method == 'POST':
        ...
        selected_post = verified_user.user_posts_set.get(
            id=captured_user_post_id)
...
def search_user(request):
    ...
    if captured_user_profile:
        try:
            selected_user_profile = User.objects.filter(
                user_name=captured_user_profile).first()
...
def like_post_other_profile(request):
    ...
    if request.method == 'POST':
        ...
        # finding post in database
        selected_post = selected_user_profile.user_posts_set.get(
            id=captured_user_post_id)
        selected_post.likes += 1
        selected_post.save()
...
def send_msg_other_profile(request):
    ...
    if request.method == 'POST':
        ...
        # saving to database
        init_contact, contact_status = Contact.objects.get_or_create(
            user_reference=selected_user_profile, contact_id=verified_user.id, contact_name=verified_user.user_name, contact_email=verified_user.user_email)
        Conversation.objects.create(contact_reference=init_contact, contact_id=init_contact.id, contact_email=init_contact.contact_email,
                                    contact_name=init_contact.contact_name, msg_content=captured_msg, has_been_read=False)
...
def validate_upload(request):
    ...
    if request.method == 'POST' and request.FILES['myfile']:
        ...
        # saving to database
        verified_user.user_photo_set.create(
            profile_picture_link=uploaded_file_url)
